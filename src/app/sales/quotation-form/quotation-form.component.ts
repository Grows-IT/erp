...
export class QuotationFormComponent implements OnInit {
  ...
  filter(val: string): string[] {
    ...
    return customersNames.filter(cus => cus.toLowerCase().indexOf(val.toLowerCase()) === 0);
  }
  ...
  onConfirmClick(status) {
    ...
    if (status === 0) {
      this.addQ = this.salesService.addQuotation(this.quotation.value).pipe(
        switchMap(() => this.salesService.getQuotation())
      ).subscribe(() =>
        this.addQ.unsubscribe()
      );
    } else if (status === 1) {
