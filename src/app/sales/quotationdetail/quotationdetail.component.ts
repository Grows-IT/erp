...
export class QuotationdetailComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.invoiceSubscription = this.invoiceService.invoices.subscribe(
      invoices => {
  ...
  getAll(itemId: string, quantity: string) {
    const all = [];
    const itemName = this.getItems(itemId);
    const itemQuan = this.getQuantity(quantity);
  ...
  getListItem(itemId, itemQuantity) {
    ...
    console.log(this.getQuantity(itemQuantity)[0]);
